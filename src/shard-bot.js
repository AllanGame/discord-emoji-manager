  ...
  GuildSchema.findOne({
    ...
    message.guild.fetchWebhooks().then(async (r) => {
      ...
      if (messageHasAttachments) {
        await hook.send(message.content.replace(regex, (_, emoji) => ` ${client.emojis.cache.find(x => x.name === emoji) === undefined ? ":" + emoji + ":" : client.emojis.cache.find(x => x.name === emoji).toString()}`), {
          "allowedMentions": { "users": [] },
          files: attachments
        })
      } else {
        await hook.send(message.content.replace(regex, (_, emoji) => ` ${client.emojis.cache.find(x => x.name === emoji) === undefined ? ":" + emoji + ":" : client.emojis.cache.find(x => x.name === emoji).toString()}`), { "allowedMentions": { "users": [] } })
      }
      message.delete()
    })
  });
