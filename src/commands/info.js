...
module.exports = {
    ...
    run: (client, message, args, storage) => {
        const has = /<a?:.+:\d+>/gm;
        const anim = /<a:.+:(\d+)>/gm;
        if (!args[0]) return message.reply("put the emoji or emoji id to get the data");
        let emoji = args[0];
        if (!has.test(emoji) && !client.emojis.cache.has(emoji)) return message.channel.send("Invalid emoji.");
        ...
        if (!client.emojis.cache.has(id)) return message.channel.send("This emoji does not exists");
